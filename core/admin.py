...
from .models import FeedGroup, Feed, Article
...
class FeedAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'aggregator', 'identifier', 'enabled')
        }),
        ('Configuration', {
            'fields': ('daily_limit', 'icon')
        }),
        ('Relationships', {
            'fields': ('user', 'group')
        }),
    ...
    def aggregate_selected_feeds(self, request, queryset):
        ...
        for feed in queryset:
            try:
                ...
                self.message_user(
                    request,
                    f"✗ Error aggregating feed '{feed.name}': {str(e)}",
                    messages.ERROR
                )
        ...
        if failed == total_feeds:
            self.message_user(
                request,
                f"All {failed} feed(s) failed to aggregate",
                messages.ERROR
            )
...
class ArticleAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        (None, {
            'fields': ('name', 'identifier', 'feed')
        }),
        ('Content', {
            'fields': ('raw_content', 'content')
        }),
        ('Metadata', {
            'fields': ('author', 'icon', 'date')
        }),
        ('Status', {
            'fields': ('read', 'starred')
        }),
    ...
    def reload_selected_articles(self, request, queryset):
        ...
        for article in queryset:
            try:
                ...
                self.message_user(
                    request,
                    f"✗ Error reloading article '{article.name}': {str(e)}",
                    messages.ERROR
                )
        ...
        if failed == total_articles:
            self.message_user(
                request,
                f"All {failed} article(s) failed to reload",
                messages.ERROR
            )
