...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    try {
      ...
      logger.debug({ userId: user.id, username: user.username }, 'User authenticated');

      // Set session
      const session = req.session as Session & { userId?: number; isSuperuser?: boolean };
      logger.debug({ sessionId: session.id }, 'Setting session properties');

      session.userId = user.id;
      session.isSuperuser = user.isSuperuser;

      logger.debug({ sessionId: session.id, userId: user.id }, 'Session properties set, saving...');
      ...
      await new Promise<void>((resolve, reject) => {
        session.save(err => {
          if (err) {
            ...
            logger.debug({ sessionId: session.id, userId: user.id }, 'Session saved successfully');
            resolve();
          }
        });
      });
