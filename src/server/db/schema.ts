...
// Feed type enum
export const feedTypeEnum = ['article', 'youtube', 'podcast', 'reddit'] as const;
...
export const feeds = sqliteTable(
  'feeds',
  {
    ...
    feedType: text('feed_type', { enum: feedTypeEnum }).notNull().default('article'),
    icon: text('icon'),
    ...
    generateTitleImage: integer('generate_title_image', { mode: 'boolean' })
      .notNull()
      .default(true),
    addSourceFooter: integer('add_source_footer', { mode: 'boolean' }).notNull().default(true),
    skipDuplicates: integer('skip_duplicates', { mode: 'boolean' }).notNull().default(true),
    useCurrentTimestamp: integer('use_current_timestamp', { mode: 'boolean' })
      .notNull()
      .default(true),
    dailyPostLimit: integer('daily_post_limit').notNull().default(50),
    aggregatorOptions: text('aggregator_options', { mode: 'json' }).notNull().default('{}'),
    aiTranslateTo: text('ai_translate_to').notNull().default(''),
    aiSummarize: integer('ai_summarize', { mode: 'boolean' }).notNull().default(false),
...
export const articles = sqliteTable(
  ...
  table => ({
    ...
    feedCreatedIdx: index('articles_feed_created_idx').on(table.feedId, table.createdAt),
    feedDateIdIdx: index('articles_feed_date_id_idx').on(table.feedId, table.date, table.id),
...
export const userArticleStates = sqliteTable(
  ...
  table => ({
    ...
    userReadIdx: index('user_article_states_user_read_idx').on(table.userId, table.isRead),
    userSavedIdx: index('user_article_states_user_saved_idx').on(table.userId, table.isSaved),
...
export const groups = sqliteTable(
  ...
  table => ({
    nameUserIdx: uniqueIndex('groups_name_user_idx').on(table.name, table.userId),
...
export const feedGroups = sqliteTable(
  ...
  table => ({
    feedGroupIdx: uniqueIndex('feed_groups_feed_group_idx').on(table.feedId, table.groupId),
...
export const userAIQuotas = sqliteTable(
  'user_ai_quotas',
  {
    ...
    monthlyResetAt: integer('monthly_reset_at', { mode: 'timestamp' }).notNull(),
    createdAt: integer('created_at', { mode: 'timestamp' })
      .notNull()
      .$defaultFn(() => new Date()),
    updatedAt: integer('updated_at', { mode: 'timestamp' })
...
export const taskExecutions = sqliteTable(
  ...
  table => ({
    ...
    executedAtIdx: index('task_executions_executed_at_idx').on(table.executedAt),
