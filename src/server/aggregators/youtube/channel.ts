 ...
 */
async function validateExistingChannelId(
  id: string,
  apiKey: string,
): Promise<string | null> {
  try {
...
function extractFromUrl(
  ...
  try {
    const urlStr = identifier.startsWith("http")
      ? identifier
      : `https://${identifier}`;
 ...
 */
async function resolveViaUsername(
  handle: string,
  apiKey: string,
): Promise<string | null> {
  try {
...
export async function resolveChannelId(
  ...
  if (!apiKey?.trim())
    return { channelId: null, error: "YouTube API key is not configured" };
  ...
  if (iden.startsWith("UC") && iden.length >= 24) {
    ...
    return validId
      ? { channelId: validId, error: null }
      : { channelId: null, error: `Channel ID not found: ${iden}` };
  }
  ...
  if (iden.includes("youtube.com") || iden.includes("youtu.be")) {
    ...
    if (extracted?.channelId)
      return resolveChannelId(extracted.channelId, apiKey);
