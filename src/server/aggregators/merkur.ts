...
export class MerkurAggregator extends FullWebsiteAggregator {
  ...
  override readonly identifierDescription = 'Select the Merkur feed to aggregate';
  ...
  override async aggregate(articleLimit?: number): Promise<RawArticle[]> {
    ...
    logger.info({ feedUrl, aggregator: this.id }, 'Using feed identifier directly');
    ...
    for (const article of articles) {
      try {
        ...
        if (!extracted || extracted.trim().length === 0) {
          logger.warn({ url: article.url }, 'Could not find .idjs-Story content, using summary');
        ...
        logger.error({ error, url: article.url }, 'Error processing Merkur article');
