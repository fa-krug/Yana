 ...
 */
export function extractContent(html: string, options: ExtractOptions = {}): string {
  ...
  try {
    ...
    if (contentSelector) {
      ...
      if (content.length === 0) {
        logger.warn({ contentSelector }, 'Content selector found no elements, using body');
        content = $('body');
      }
    } else {
      // Try common content selectors
      content =
        $('article, .article, .post, .entry, .content, main, #content').first() || $('body');
    }
