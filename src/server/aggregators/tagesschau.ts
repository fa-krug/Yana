...
export class TagesschauAggregator extends FullWebsiteAggregator {
  ...
  protected override shouldSkipArticle(article: RawArticle): boolean {
    // Check title filters
    const skipTerms = ['tagesschau', 'tagesthemen', '11KM-Podcast', 'Podcast 15 Minuten'];
  ...
  private extractMediaHeader(soup: cheerio.CheerioAPI): string | null {
    ...
    const playersToCheck = teaserPlayers.length > 0 ? teaserPlayers : mediaPlayers;
    ...
    for (let i = 0; i < playersToCheck.length; i++) {
      ...
      try {
        ...
        const isAudioOnly =
          streams.length > 0 && streams.every((stream: any) => stream.isAudioOnly === true);

        // Extract image/poster from player data
        let imageUrl: string | null = null;
        // Check common image fields in mc
        const imageFields = ['poster', 'image', 'thumbnail', 'preview', 'cover'];
        ...
        for (const stream of streams) {
          ...
          for (const mediaItem of mediaItems) {
            ...
            } else if (!isAudioOnly && mimeType.toLowerCase().includes('video')) {
    ...
    for (const article of articles) {
      try {
        ...
        if (mediaHeader) {
          ...
          logger.debug({ url: article.url }, 'Prepended media header to article content');
        }

        // Standardize format (add source link, but skip title image if media header exists)
        const generateTitleImage = !mediaHeader && (this.feed?.generateTitleImage ?? true);
        ...
        logger.error({ error, url: article.url }, 'Error processing Tagesschau article');
