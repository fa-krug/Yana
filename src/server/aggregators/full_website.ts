...
export class FullWebsiteAggregator extends BaseAggregator {
    ...
    for (let i = 0; i < itemsToProcess.length; i++) {
      ...
      try {
        ...
        if (ignoreTitle) {
          ...
          if (
            titleFilters.some(filter => article.title.toLowerCase().includes(filter.toLowerCase()))
          ) {
            logger.debug(
              {
                index: i + 1,
        ...
        const ignoreContent = this.getOption('ignore_content_contains', '') as string;
        if (ignoreContent) {
          ...
          if (contentFilters.some(filter => searchText.includes(filter.toLowerCase()))) {
            logger.debug(
              {
                index: i + 1,
                title: article.title,
        ...
        try {
          ...
          const additionalSelectors = excludeSelectors
            ...
            .map(line => line.trim())
            .filter(line => line.length > 0);
          const allSelectorsToRemove = [...this.selectorsToRemove, ...additionalSelectors];
          ...
          if (regexReplacements) {
            processedContent = this.applyRegexReplacements(processedContent, regexReplacements);
          }
   ...
   */
  private applyRegexReplacements(content: string, regexReplacementsText: string): string {
