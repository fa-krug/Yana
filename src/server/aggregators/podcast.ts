...
function extractEnclosure(item: any): { url: string; type: string } {
  ...
  if (item.enclosures && Array.isArray(item.enclosures) && item.enclosures.length > 0) {
    for (const enclosure of item.enclosures) {
      ...
      if (mediaType.startsWith('audio/') || /\.(mp3|m4a|wav|ogg|opus|aac)$/i.test(url)) {
...
export class PodcastAggregator extends BaseAggregator {
  ...
  override async aggregate(articleLimit?: number): Promise<RawArticle[]> {
    ...
    logger.info({ feedUrl, itemCount: itemsToProcess.length }, 'Processing podcast episodes');
    ...
    for (const item of itemsToProcess) {
      try {
        ...
        if (!audioUrl) {
          logger.warn({ title: item.title }, 'Podcast episode has no audio enclosure, skipping');
          continue;
        }
        ...
        if (duration) {
          ...
          htmlParts.push(`<span class="podcast-duration">${formattedDuration}</span>`);
        }
    ...
    logger.info({ feedUrl, articleCount: articles.length }, 'Podcast aggregation complete');
