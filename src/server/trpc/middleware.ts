 ...
 */
export function requireSuperuser(ctx: Context): asserts ctx is Context & {
  user: NonNullable<Context["user"]> & { isSuperuser: true };
} {
  requireAuth(ctx);
...
export function getSuperuser(
  ctx: Context,
): NonNullable<Context["user"]> & { isSuperuser: true } {
