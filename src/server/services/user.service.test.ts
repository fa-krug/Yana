...
import { setupTestDb, teardownTestDb, getTestDb } from '../../../tests/utils/testDb';
import { createUser, authenticateUser, getUserById, getUserByUsername } from './user.service';
...
describe('UserService', () => {
  ...
  describe('createUser', () => {
    it('should create a new user', async () => {
      const user = await createUser(testUser.username, testUser.email, 'password');
    ...
    it('should throw error if email already exists', async () => {
      ...
      await expect(createUser('otheruser', testUser.email, 'password')).rejects.toThrow();
    });
  });
  ...
  describe('authenticateUser', () => {
    ...
    it('should throw error with incorrect password', async () => {
      ...
      await expect(authenticateUser(testUser.username, 'wrongpassword')).rejects.toThrow();
    });

    it('should throw error for non-existent user', async () => {
      await expect(authenticateUser('nonexistent', 'password')).rejects.toThrow();
    });
  });

  describe('getUserById', () => {
    it('should get user by ID', async () => {
      const created = await createUser(testUser.username, testUser.email, 'password');
