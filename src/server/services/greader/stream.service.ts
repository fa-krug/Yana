...
export async function getStreamContents(
  ...
  const readIdArray =
    excludeTag === STATE_READ
      ? await fetchStateArticleIds(userId, STATE_READ)
      : [];
  ...
  const baseConditions = [accessControl, eq(feeds.enabled, true)].filter(
    Boolean,
  ) as SQL<unknown>[];
 ...
 */
async function fetchStateArticleIds(
  userId: number,
  stateKey: string,
): Promise<number[]> {
 ...
 */
async function getStarredStreamQuery(
  userId: number,
  readIdArray: number[],
  ...
  const accessibleFeeds = await db
    ...
    .where(
      and(
        ...([accessControl, eq(feeds.enabled, true)].filter(
          Boolean,
  ...
  if (readIdArray.length > 0)
    conditions.push(notInArray(articles.id, readIdArray));
...
/**
 * Parse timestamp from olderThan string.
 */
function _parseOlderThanTimestamp(olderThan: string): Date | null {
...
/**
 * Build base article query conditions.
 */
function _buildBaseQueryConditions(
  userId: number,
  readIdArray: number[],
  starredIdArray: number[],
  timestampDate: Date | null,
  ...
  if (starredIdArray.length > 0)
    conditions.push(inArray(articles.id, starredIdArray));
  if (timestampDate) conditions.push(lt(articles.date, timestampDate));
  return conditions;
}
...
export async function getStreamItemIds(
  ...
  const readIdArray =
    excludeTag === STATE_READ
      ? await fetchStateArticleIds(userId, STATE_READ)
      : [];
  const starredIdArray =
    includeTag === STATE_STARRED
      ? await fetchStateArticleIds(userId, STATE_STARRED)
      : [];
  if (includeTag === STATE_STARRED && starredIdArray.length === 0)
    return { itemRefs: [] };

  const timestampDate = _parseOlderThanTimestamp(olderThan);

  const baseConditions = _buildBaseQueryConditions(
    userId,
    readIdArray,
    starredIdArray,
    timestampDate,
  ...
  if (streamId === STATE_STARRED) {
    const starredQuery = await getStarredStreamQuery(
      userId,
      readIdArray,
      timestampDate,
    );
    if (!starredQuery) return { itemRefs: [] };
    articleQuery = starredQuery as unknown as typeof articleQuery;
  } else if (streamId && streamId !== STATE_READING_LIST) {
    const filtered = await filterArticlesByStream(
      articleQuery as unknown as Record<string, unknown>,
      streamId,
      userId,
    );
    articleQuery = filtered as unknown as typeof articleQuery;
  }
...
function formatUnreadCount(
  ...
  if (unreadCount > 0 || includeAll) {
    ...
    return {
      id: `feed/${stat.feedId}`,
      count: unreadCount,
      newestItemTimestampUsec: newestDate
...
export async function getUnreadCount(
  ...
  for (const stat of feedStats) {
    const formatted = formatUnreadCount(
      stat,
      readCountMap.get(stat.feedId) || 0,
      includeAll,
    );
    if (formatted) {
