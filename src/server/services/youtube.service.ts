    ...
    if (axios.isAxiosError(error)) {
      ...
      if (error.response?.status === 400) {
        ...
        if (errorData?.error?.errors?.[0]?.reason === 'quotaExceeded') {
          errors.general = 'YouTube API quota exceeded. Please try again later.';
        } else if (errorData?.error?.errors?.[0]?.reason === 'accessNotConfigured') {
          errors.general = 'YouTube Data API v3 is not enabled. Enable it in Google Cloud Console.';
        } else {
        ...
        errors.general = 'Cannot connect to YouTube API. Please check your internet connection.';
      } else {
        // Other HTTP errors
        const errorData = error.response?.data;
        errors.general =
